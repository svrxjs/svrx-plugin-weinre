...
var HttpChannelHandler, Version, channelManager, checkForDeath, checkHost, deathTimeout, dns, dumpingHandler, express, fs, getStaticWebDir, getVersion, jsonBodyParser, net, path, processOptions, run2, serviceManager, startDeathWatcher, startServer, utils, _, favicon, errorhandler, serveStatic;
...
startServer = function() {
  ...
  // app.use(express["static"](options.staticWebDir, staticCacheOptions));
  app.use(serveStatic(options.staticWebDir, staticCacheOptions));
  if (options.boundHost === '-all-') {
    utils.notify("starting server at http://localhost:" + options.httpPort);
    return app.listen(options.httpPort);
  } else {
    utils.notify("starting server at http://" + options.boundHost + ":" + options.httpPort);
